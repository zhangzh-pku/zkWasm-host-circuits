...
#[cfg(test)]
mod tests {
    use super::{Bls381ChipConfig, Bls381PairChip};
    use crate::utils::Limb;
    use halo2_proofs::arithmetic::FieldExt;
    use halo2_proofs::circuit::floor_planner::FlatFloorPlanner;
    use halo2_proofs::dev::MockProver;
    use halo2_proofs::pairing::bls12_381::{pairing, Fq, G1Affine, G2Affine, Gt, G1, G2};
    use halo2_proofs::{
        circuit::Layouter,
    ...
    fn fq_to_biguint(fq: &Fq) -> BigUint {
        BigUint::from_bytes_be(&fq.to_bytes())
    }
    ...
    fn gt_to_limbs(gt: &Gt) -> Vec<Limb<halo2_proofs::pairing::bn256::Fr>> {
        let fq12 = &gt.0;
        let coeffs = [
            fq12.c0.c0.c0,
            fq12.c0.c0.c1,
    ...
    impl Circuit<halo2_proofs::pairing::bn256::Fr> for PairCircuit {
        ...
        fn configure(meta: &mut ConstraintSystem<halo2_proofs::pairing::bn256::Fr>) -> Self::Config {
            Bls381PairChip::<halo2_proofs::pairing::bn256::Fr>::configure(meta)
        }
        ...
        fn synthesize(
            ...
            let chip = Bls381PairChip::construct(config);
            chip.load_bls381_pair_circuit(&self.a, &self.b, &self.ab, &mut layouter)
        }
    }

    #[test]
    fn test_bls381_pairing_constraints() {
